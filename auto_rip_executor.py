...
def start_auto_transfer(base_folder, folder_names, hotfolder_paths, auto_trigger,
    ...
    for folder in priority_list:
        if status_callback:
            status_callback(folder, "대기", progress=0, elapsed=None)
        ...
        common_names = natsorted(common_names, reverse=reverse)
        ...
        if status_callback:
            status_callback(folder, "전송 중", progress=0, elapsed=0)
        ...
        for idx, base_name in enumerate(common_names, start=1):
            ...
            try:
                ...
                if status_callback:
                    ...
                    elapsed = int(time.time() - folder_start)
                    status_callback(folder, f"전송 중: {base_name}", progress=progress, elapsed=elapsed)
        ...
        if status_callback:
            elapsed_total = int(time.time() - folder_start)
            status_callback(folder, "완료", progress=100, elapsed=elapsed_total)
